...
public class OperationProcessor implements SerialPortEventListener {
	 ...
	 * @param error
	 */
	public void writeData(int direction, int velocity, int time, int error) {
		try {
			String timeStr = String.format("%03d", time);
			//Sd0v0t123e0E
			//S_DIR_VELOCITY_TIME_ERROR_E
			// Send the message.
			outputStream.flush();
			String toSend = "S".concat("d" + direction + "v" + velocity + "t" + timeStr + "e" + error + "E");
			...
			if (error == 0) {
				...
				log.log("Pumps stopped by user!", "Pumps stopped by user!");
				logger.info("Pumps stopped by user!");
			}
		} catch (Exception e) {
	...
	public void stopRobotMovement() {
		/* Send a stop message to uC */
		writeData(0, 0, 0, 0);
	}
	
	public void stopEmergencyRobotMovement() {
		/* Send a stop message to uC */
		writeData(0, 0, 0, 1);
	}
